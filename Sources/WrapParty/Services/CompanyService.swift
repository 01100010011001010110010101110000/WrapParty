...
extension CompanyService {
  enum Router: RequestRoutable {
    ...
    func asUrl() -> URL {
      switch self {
      case let .details(id):
        return componentsForRoute(path: "company/\(id)").url!
      case let .alternativeNames(id):
        return componentsForRoute(path: "company/\(id)/alternative_names").url!
      case let .images(id):
        return componentsForRoute(path: "company/\(id)/images").url!
      }
    }
  }
}
